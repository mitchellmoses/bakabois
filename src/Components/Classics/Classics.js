import React, { useState, useEffect } from 'react';
import './Classics.css';
import img_3690 from "../../assets/image/IMG_3690.jpg";

function Classics() {
  const [userMemes, setUserMemes] = useState([]);

  useEffect(() => {
    // Load user memes from local storage on component mount
    const storedMemes = JSON.parse(localStorage.getItem('classicsUserMemes')) || [];
    setUserMemes(storedMemes);
  }, []);

  const handleFileUpload = (event) => {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        const newMeme = { id: Date.now(), src: reader.result };
        const updatedMemes = [...userMemes, newMeme];
        setUserMemes(updatedMemes);
        localStorage.setItem('classicsUserMemes', JSON.stringify(updatedMemes));
      };
      reader.readAsDataURL(file);
    }
  };

  return (
    <div className="classics-container">
      <h1 className="classics-title">Fantasy Football Classic Moments</h1>
      
      <div className="user-memes-section">
        <h2>User Submitted Memes</h2>
        <input type="file" accept="image/*" onChange={handleFileUpload} />
        <div className="user-memes-grid">
          {userMemes.map((meme) => (
            <img key={meme.id} src={meme.src} alt="User submitted meme" className="user-meme" />
          ))}
        </div>
      </div>

      <div className="classics-image-container">
        <img src={img_3690} alt="Fantasy Football Classic Moment" className="classics-image" />
      </div>
      <p className="classics-description">Relive the greatest moments in our fantasy football history!</p>
    </div>
  );
}

export default Classics;